class noiseWalker {
  ...
  noiseWalker(float _angle) {
    ...
    incX = random(0.01, 0.07);
    incY = random(0.01, 0.07);
  ...
  void drawNoiseWalker() {
    ...
    //x = x + mappedNoiseX; // 12
    //y = y + mappedNoiseY; // 22
    // Calculate total distance using Pvector
    PVector p1 = new PVector(width*0.5, height*0.5);
    PVector p2 = new PVector(x, y);
    if (p1.dist(p2) < radRoomCircle / 2) 
    {
      x = x + cos(radians(angle) + mappedNoiseX);
      y = y + sin(radians(angle) + mappedNoiseX);
    }
    /*if (x > width) {
     x = 0;
     pX = x;
