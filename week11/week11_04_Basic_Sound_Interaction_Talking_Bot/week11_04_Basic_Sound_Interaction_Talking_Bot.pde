...
// smooth factor towards 0, the mea sured amplitudes are averaged across frames,
...
void setup() {
  ...
  sample = new SoundFile(this, "text_to_speech.aiff");
  ...
  // Initialize bot object and set it's location on canvas
  bot = new Robot();
  bot.x = width / 2 - bot.robotHeadW / 2;
  bot.y = height / 2 - bot.robotHeadH / 2;
}
...
void draw() {
  ...
  if (isRMSSmoothed == true) 
  {
    ...
    radius = map(rms.analyze(), 0, 1, 0, 100);
  }
  
  // Set the height of the Robot's moutH to radius
  bot.mouthH = radius;
