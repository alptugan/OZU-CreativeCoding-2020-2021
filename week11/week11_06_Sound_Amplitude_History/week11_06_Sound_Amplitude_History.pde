...
void draw() {
  ...
  // Draw a shape to show the flowing line which is generated from amplitude values according to passed time
  beginShape();
  for(int i = 0; i < rmsHistory.size(); i++) {
    float mappedVal = map(rmsHistory.get(i), 0, 1, height, 0);
    vertex(i, mappedVal);
  }
  endShape();
